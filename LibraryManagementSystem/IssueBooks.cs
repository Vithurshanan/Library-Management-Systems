...
namespace LibraryManagementSystem
{
    public partial class IssueBooks : UserControl
    {
                ...
                if (connect.State != ConnectionState.Open)
                {
                    try
                    {
                        ...
                        string insertData = "INSERT INTO issues " +
                            "(issue_id, full_name, contact, email, books_title, author, status, issue_date, return_date, date_insert) " +
                            "VALUES(@issueID, @full_name, @contact, @email, @bookTitle, @author, @status, @issueDate, @returnDate, @dateInsert)";
        ...
        {
            // Add any required code here or leave it empty if it's not used.
        }
                    ...
                    if (check == DialogResult.Yes)
                    {
                        try
                        {
                            ...
                            string updateData = "UPDATE issues SET full_name = @fullName, contact = @contact, email = @email" +
                                ", books_title = @bookTitle, author = @author, status = @status, issue_date = @issueDate" +
